...
下面是一个更详细的例子。我们将通过一个简单的测试第一个练习。目标是创建一个字符串实用程序库。
首先，让我们编写一个将以空结尾的字符串转换为所有大写的函数。

Ensure that CuTest.c and CuTest.h are accessible from your C
project. Next, create a file called StrUtil.c with these
contents:

确保可以从C项目访问cutest.c和cutest.h。接下来，创建一个名为strutil.c的文件，其中包含以下内容：
*****************************StrUtil.c*****************************
    #include "CuTest.h"
   ...
   #类似与创建一个测试实例
    CuSuite* StrUtilGetSuite() {
        ...
        /*
	add other test
	 SUITE_ADD_TEST(suite, TestStrToUpper_NULL);
        */
        return suite;
    }
...
在编写这些测试的每一个时，将其添加到strutilgetsuite函数中。
如果没有，测试就不会运行。稍后，当您编写其他函数并为它们编写测试时，
请确保在strutilgetsuite中也包含这些函数。
strutilgetsuite函数应该包括strutil.c中的所有测试。
