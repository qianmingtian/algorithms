...
unsigned long int diff_time(unsigned long int t2, unsigned long int t3)
{
    struct timeval time_2, time_3, tem;
    unsigned long int diff_result;
    if (t3 >= t2)
    {
        time_2 = time_tv_decode(t2);
        time_3 = time_tv_decode(t3);

        tem.tv_sec = time_3.tv_sec - time_2.tv_sec;
        tem.tv_usec = time_3.tv_usec - time_2.tv_usec;

        //时间秒级跃变
        diff_result = tem.tv_sec * 999999 + tem.tv_usec;
        return diff_result;
    }
    else
    {
#ifdef debug
        printf("condition: t3>t2 is not satisfied\n");
        exit(-215);
#endif
    }
}
